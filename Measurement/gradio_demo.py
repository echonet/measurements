...
"""
Purpose:
    Interactive Gradio demo for Doppler peak velocity prediction (TRVmax by default).
    Upload a DICOM file and the model predicts the peak velocity and overlays the
    predicted point on the Doppler image.

Input:
    - DICOM file (.dcm) containing a Doppler region (CW/PW)

Output:
    - Predicted peak velocity (cm/s)
    - Annotated Doppler image (PNG)

Usage:
    # Run as a script to launch the Gradio web interface:
    python gradio.py
    # Or copy this file into a Jupyter notebook and run cell by cell.
    # The default model is TRVmax; change weights_path to switch models.
"""
...
from utils import segmentation_to_coordinates, get_coordinates_from_dicom
...
REFERENCE_LINE_TAG = (0x0018, 0x6022)  # Doppler baseline Y position
